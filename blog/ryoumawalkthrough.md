# Ryoma Echizen

This guide tells you where to find your tennis boy on the map, and what interactions you can have with him at what time!

Necessary interactions are in **bold**, non-bold interactions are optional but recommended.

NOTE: Ryoma has different versions of CGs that you can get on a second playthrough, this guide will be updated at a later date to include what is needed to unlock those. Right now we’re just trying to get everyone’s guide published!

Morning greeting: “Good morning!”

---

## 8/22

- Morning: Indoor Court → Chat → Any Topic
- **Day: Square → Chat → “Now I’m curious…what breed is it?”** *Special Topic “Karupin” unlocked*
- Evening: Indoor Court → Chat →  Any Topic *(cannot choose “Karupin” yet)*
## 8/23

- Morning: Indoor Court → Chat → Any Topic *(Karupin option now available)*
- **Day: Square → Look → “Wake him up”**
- Evening: Main Building → Chat → Any Topic
## 8/24

- Morning:  Stage → Chat → Any Topic
- **Day:  Main Building → Look** → Chat → Any Topic
- **Evening:  Main Building → Chat → Auto**
## 8/25

- **Morning:  Square → Look**
- **Day:  Indoor Court → Chat→ Auto**
- Evening:  Main Building → Chat → Any Topic
## 8/26

- Morning:  Main Building → Look → Mini Game
- **Day:  Square → Look** → Chat → Any Topic
- **Evening:  Main Building → Chat → “Sure, let’s go!” → “We can’t let him do this.”**
## 8/27

- **Morning:  Main Building → Look** → Chat → Any Topic
- Day:  Square → Chat → Any Topic
- **Evening:  Square → Look (at Kintaroh Toyama)** → Indoor Court  → Chat → Any Topic
## 8/28

**:two_hearts:** **DATE DAY: “What?! Oh no!” → “The miso mackerel meal.”**
## 8/29

- **Morning: Main Building → Chat → Auto**
- **Day: Main Building → Chat → Auto**
- Evening: Stage → “Sure” or Chat → Any Topic
## 8/30

- **Morning:  Main Building → Look**
- Day:  Main Building → “Sure” or Chat → Any Topic
- **Evening:  Square → Chat → Auto**
## 8/31

**:two_hearts:** **DATE DAY: Any Choice**
## 9/1

- No choices
## 9/2

- Morning:  Square → Chat → Auto
- **Day:  :tada: Special Event**
- Evening: No Choices
## 9/3 - Festival Day 1

- **Morning: Main Building → Look → Call Out → Ryoma Echizen → “Do you need help, Ryoma?”**
- **Day Festival Performance:  “Yeah, sure!”**
- Evening:  No choices
## 9/4 - Festival Day 2

- Morning: Free Choice
- Day: Free Choice
- Evening: Free Choice
 
